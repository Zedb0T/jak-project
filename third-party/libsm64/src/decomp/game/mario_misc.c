 ...
 * libsm64 fork: whitelist extended with ACT_IDLE and ACT_WAITING_FOR_DIALOG so the
 * host puppet driver (which forces ACT_IDLE while a Jak cutscene plays) can write
 * torsoAngle without having it zeroed out every frame. Stock sm64 has no plain
 * ACT_CUTSCENE or ACT_DRIVING_VEHICLE — the real cutscene-group actions are the
 * dialog/star/intro/credits variants, and the host only ever forces ACT_IDLE.
 */
Gfx *geo_mario_tilt_torso(s32 callContext, struct GraphNode *node, UNUSED Mat4 *c) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        ...
        if (action != ACT_BUTT_SLIDE && action != ACT_HOLD_BUTT_SLIDE && action != ACT_WALKING
            ...
            && action != ACT_IDLE && action != ACT_WAITING_FOR_DIALOG) {
...
// libsm64 fork: per-limb tilt hooks for the arm chain. Read from bodyState
// semantic angles (leftArmAngle, leftForeArmAngle, etc.) that the host code
// writes via sm64_set_mario_*_angle. Storage order is (Y, Z, X) on the rot
// node — the same swap geo_mario_tilt_torso uses.
Gfx *geo_mario_tilt_left_arm(s32 callContext, struct GraphNode *node, UNUSED Mat4 *c) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        struct GraphNodeRotation *rotNode = (struct GraphNodeRotation *) node->next;
        rotNode->rotation[0] = bodyState->leftArmAngle[1];
        rotNode->rotation[1] = bodyState->leftArmAngle[2];
        rotNode->rotation[2] = bodyState->leftArmAngle[0];
    }
    return NULL;
}
...
/**
 * Makes Mario's head rotate based on bodyState->headAngle.
 * libsm64 fork: uncommented the rotation-assignment path so the host puppet
 * driver can set headAngle via sm64_set_mario_headangle and have it show up.
 */
