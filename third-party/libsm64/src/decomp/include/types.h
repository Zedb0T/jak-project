...
struct MarioBodyState
{
    ...
    u8 padding[4];
    // libsm64 fork extension: per-limb tilt angles written by the public
    // sm64_set_mario_*_angle setters and read by the geo_mario_tilt_* ASM
    // nodes at render time. Each one is an XYZ Euler in Mario's s16 angle
    // units (0x10000 = 2pi). Not part of the stock N64 MarioBodyState — the
    // stock layout above is preserved verbatim for N64-compat; these new
    // fields are appended so the offset comments remain accurate.
    Vec3s leftArmAngle;
    Vec3s rightArmAngle;
    Vec3s leftForeArmAngle;
    Vec3s rightForeArmAngle;
    Vec3s leftHandAngle;
    Vec3s rightHandAngle;
};
