...
// Parse one anim entry starting at `read_ptr`. `size` is the containing blob
// size (used when the end_offset field is 0, which is the case for custom
// single-anim blobs). For ROM anims passed via load_mario_anims_from_rom the
// end_offset is always nonzero and size is ignored.
...
// Append a custom animation to the end of the global anim table and return
// its new index. Blob format mirrors a ROM anim-table entry with a 24-byte
// header (the header is skipped — actual anim data starts at data+24).
